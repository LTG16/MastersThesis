...
library(imputeTS)
library(geosphere)##########
##### In this code we use the F matrices, which we obtained in "ExtendingFGibbs"
...
#### here we initialise the fixed values, for F we need to run the code "ExtendingFGibbs" before this one

H<-diag(length(sites))
phi3<-0.1595736
sigma_eta3<-0.2031234
R3<-0.03709048*diag(length(sites))
#####################################################################################
y4<-log(data4)
