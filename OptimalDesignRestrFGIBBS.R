##########
####Optimal design for Gibbs-Sampler values and restricted F
...
library(imputeTS)
library(geosphere)
##### We start again by downloading the necessary datasets and initialising them
...
#####################################################################################
y3<-log(data3)
F3<-0.9849384*diag(length(sites))
H<-diag(length(sites))
phi3<-0.2657383
sigma_eta3<-0.2011641
R3<-0.06100765*diag(length(sites))
#####################################################################################
y4<-log(data4)
